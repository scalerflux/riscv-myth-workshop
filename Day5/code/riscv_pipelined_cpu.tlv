...
\TLV
   ...
   |cpu
      ...
      @1   
         ...
         // Immediate Decode - FIXED VERSIONS
         $imm_valid = $is_i_instr || $is_s_instr || $is_u_instr || $is_b_instr || $is_j_instr;
      ...
      @3 
         ...
         //DISABLE write for current load ($is_load), but ENABLE for returning load (>>2$valid_load)
         $rf_wr_en = ($valid && $rd_valid && ($rd != 5'b0) && !$is_load) || 
                  >>2$valid_load;
